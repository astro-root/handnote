...
import { Stroke, NoteImage, Tool, PageBackground, Page, PaperSize, Orientation } from '../types'
...
export function PageSwiper({
  ...
  const curlRef         = useRef<CurlHandle>(null)
  ...
  const swipePR = useRef(
    PanResponder.create({
      ...
      onMoveShouldSetPanResponder: (_, gs) =>
        Math.abs(gs.dx) > 40 && Math.abs(gs.dx) > Math.abs(gs.dy) * 2,
      ...
      onPanResponderRelease(_, gs) {
        const idx = pageIdxRef.current, len = pagesRef.current.length
        const threshold = 55
        if (gs.dx < -threshold && idx < len - 1) flipTo(idx + 1)
        else if (gs.dx > threshold && idx > 0)   flipTo(idx - 1)
      },
    })
  ).current
