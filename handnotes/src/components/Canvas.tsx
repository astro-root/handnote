...
import Svg, { Path, Image as SvgImage, Line, Circle, Defs, Mask, Rect, G } from 'react-native-svg'
...
export function Canvas({ strokes, images, tool, color, strokeWidth, background, onAdd, onRemove }: Props) {
      ...
      onPanResponderGrant(e) {
        ...
        if (t === 'eraser') {
          ...
          const hit = strokesRef.current
            .filter(s => s.tool !== 'eraser-pixel' && s.points.some(p => (p.x - x) ** 2 + (p.y - y) ** 2 < r * r))
            .map(s => s.id)
